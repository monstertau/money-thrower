...
export class CategoryAddComponent implements OnInit {
    ...
    constructor(private fb: FormBuilder, private categorryService: CategoryService, private notification: NzNotificationService) {
    }
    ...
    submitAddCategory() {
        ...
        this.isLoading = true;
        var category = this.form.value;
        if (this.form.controls.name.value == null)
            this.notification.error('Error', "Please input your category name");
        else {
            this.categorryService.addCategory(category).subscribe(result => {
                if (result == 'SUCCESS') {
                    this.notification.success('Success', 'Add Category Success');
                    setTimeout(() => {
                        window.location.href = '/my-categories';
                    }, 1000);
                } else {
                    this.notification.error('Error', 'Add Categoty Fail');
                }
            }, (message) => {
